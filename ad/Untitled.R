...
library(readr)
table13_cities <- read_csv("~/ad/table13_cities.csv", 
                           col_types = cols(date = col_date(format = "%Y")))
library(readr)
table14_cities <- read_csv("~/ad/table14_cities.csv", 
                           col_types = cols(date = col_date(format = "%Y")))


vars <- c(
  "Disability" = "disability.df",
  "Ethnicity" = "ethnicity.df",
  "Gender" = "gender.df",
  "Gender Identity" = "genderidentity.df",
  "Race" = "race.df",
  "Race/Ethnicity/Ancestry" = "raceethnicityancestry.df", 
  "Religion" = "religion.df",
  "Sexual Orientation" = "sexualorientaion.df"
)

vars2 <- c(
  "Did Not Report" = "didnotreport.df" 
  
)
  
ui <- navbarPage("ADL Hate Crime Mapping and Visualizations Beta", id="nav",
  tabPanel('Table 13: Bias Motivation Map',
           div(class='outer', 
               tags$head(
                 # Include custom CSS
               ...
               absolutePanel(id = "controls", class = "panel panel-default", fixed = TRUE,
                             ...
                             width = "400", height = "auto", h2("Hate Crimes by Bias Motivation"), p("Use the time slider and input box below to explore hate crime statistics from the FBI's Table reports on hate crimes from 2004-2015. "),
                sliderInput("range", "Years:", min(table13_cities$date), max(table13_cities$date),
                            value = range(table13_cities$date), step = 367, timeFormat = "%Y", round = TRUE, dragRange = TRUE
                ),
               selectInput("year", "Hate Crime Bias Motivation:", vars, selected = "Disability" )
                )
               
)), 
tabPanel('Table 14: Hate Crime Zero Map',
         div(class='outer', 
             tags$head(
               # Include custom CSS
               includeCSS('style.css')
             ...
             absolutePanel(id = "controls", class = "panel panel-default", fixed = TRUE,
                           draggable = TRUE, top = 120, left = 80, right = "auto", bottom = "auto",
                           width = "400", height = "auto", h2("Did Not Report and Zero Hate Crime Data for U.S. Cities"), p("Use the time slider and input box below to explore hate crime statistics from the FBI's Table reports on hate crimes from 2004-2015. "),
                           sliderInput("range2", "Years:", min(table14_cities$date), max(table14_cities$date),
                                       value = range(table14_cities$date), step = 367, timeFormat = "%Y"
                           ),
